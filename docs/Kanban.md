...
- [ ] Disputas — painel de mediação sai de `/admin/disputes` (papel errado: mediação exige `MEDIATOR`, não `ADMIN`) e passa a chamar `GET /disputes`, `GET /disputes/:id`, `POST /disputes/:id/evidence`/`messages`/`decision`; decisão passa a exigir **dois mediadores diferentes** (recomendação + aprovação), fluxo assíncrono, não uma tela só. A abertura (`POST /orders/:id/dispute`, do lado de quem está na ordem) já está real — ver [[14 - Ofertas e Ordens]] #jose
- [ ] Aposentar `getUserReputation` local (`src/lib/mock/reputation.ts`) em favor de `GET /users/:id/ratings` (já com cliente real em `src/lib/ratings/api.ts`, só ainda sem chamador) — usado hoje em `/profile`, `offer-list.tsx` e `counterparty-reputation-panel.tsx`, todos ainda no cálculo local a partir do mock de ordens #julia
- [ ] **Bloqueio novo, achado ao ligar leitura real de ordens**: toda checagem "sou eu o cliente/caixeiro desta ordem" no app compara contra `useMockSession().user.id` (identidade "vista como" do `AccountSwitcher`), não contra a identidade real do JWT — com ordens de verdade, uma conta real vê os controles do papel errado (testado: autor da própria ordem via API real via `cliente@teste.local` via UI mostrava o botão "Aceitar ordem", que o backend corretamente recusa, mas nem deveria aparecer). Correção de fundo é aposentar `MockSessionProvider` inteiro (perfil, chaves PIX, limite de caixeiro) em favor do JWT — maior que este bucket sozinho; ver [[14 - Ofertas e Ordens]] #jose
