...
## Ciclo de vida da ordem, cancelamento e avaliação — API real

Rodada seguinte: fecha o resto do bucket "Ordens & Ofertas" (Sprint 4) que ainda estava em `src/lib/mock/orders.tsx` — leitura (`GET /orders`, `GET /orders/:id`), PIX pós-aceite, ciclo de transferência/confirmação, cancelamento e avaliação, todos contra `marinsprosper-api` real (repositório separado, clonado localmente só pra inspeção de código — controllers, DTOs e `prisma/schema.prisma` — não como dependência de build). Disputas (mediação, painel admin) ficam pra uma rodada futura — só a abertura (`POST /orders/:id/dispute`, do lado de quem está na ordem) entrou aqui.

**Decisões confirmadas com o usuário antes de implementar:**

1. **Sem nome de contraparte** — o backend real não expõe nenhum endpoint público de perfil de usuário (nem `GET /users/:id`, nem embutido na resposta de ordem/avaliação). Toda tela que hoje mostrava nome passa a mostrar um identificador curto derivado do id (`Cliente 8F3A21C0`) — decisão explícita do usuário, não um placeholder "TODO".
2. **Negociação de oferta cria ordem com termos normais** (já registrado na seção anterior) — mantido.

### `presentOrderForFrontend` (`src/lib/orders/adapt.ts`)

Converte `BackendOrder` (espelho de `present(order)` do `orders.service.ts` real — `src/lib/orders/types.ts`) pro formato rico `Order` do protótipo (`src/types/order.ts`), reaproveitando `mapBackendOrderStatus` (existia desde a rodada de remapeamento da máquina de estados, mas ainda não tinha nenhum chamador real). Isso permite que `OrderTimeline`/`OrderStatusBadge`/as listagens continuem funcionando sem reescrever — só a fonte dos dados muda. Campos que a API real não tem: nome (ver acima), comprovante/TXID informado por usuário (os endpoints de transição não recebem corpo — evidência vive no chat/disputa, buckets ainda não integrados), motivo de cancelamento/disputa (o backend recebe mas não devolve no corpo da ordem) e snapshot de chave PIX (a ordem real tem só uma `pixKey`/`pixDocument`, de quem recebe, não duas). `txid` de exibição passa a vir de `settleTxHash` (liquidação real, quando existir), não mais digitado pelo usuário.

### `GET /orders` / `GET /orders/:id`

`orders/page.tsx` e `order-detail.tsx` (via `OrderDetail`) trocaram de `useMockOrders()` pra `listOrdersRequest`/`getOrderRequest` — `useEffect` com IIFE async inline + flag `cancelled` (não uma função nomeada chamada de dentro do efeito — o lint novo do `eslint-plugin-react-hooks`, `set-state-in-effect`, reprova isso mesmo com o `setState` depois de um `await`). Refetch depois de uma ação usa um `refetchToken` incrementado por estado, não uma segunda chamada de função dentro de outro efeito. IDOR: a checagem de participante passou a ser só a do backend (`assertCanRead` — parte, mediador, ou caixeiro pra ordem `OPEN`; 404 tanto pra "não existe" quanto "não é sua", de propósito).

### Ciclo de transferência (`OrderActions`, reescrito)

Mais simples que a versão mock: os endpoints reais (`client-transfer`, `cashier-confirm-receipt`, `cashier-transfer`, `client-confirm`) não recebem corpo — sem upload de comprovante, sem campo de TXID digitado. Ganho novo que a versão mock não tinha: a etapa `ACCEPTED` real exige que quem VAI RECEBER em BRL registre a própria chave PIX (`POST /orders/:id/pix`, `RegisterPixControl`) antes de qualquer transferência — quem recebe depende do lado (`CLIENT_BUYS_ASSET` → cashier recebe; `CLIENT_SELLS_ASSET` → cliente recebe), e o documento é conferido contra o KYC de quem registra (mesma trava anti-triangulação, agora do lado do backend de verdade). `PaymentCountdown` foi mantido (mesma UX visual), mas `onExpire` agora só dispara um refetch — o timeout automático é responsabilidade do backend (`order-deadline.sweeper.ts`, fila real), não mais um evento disparado pelo cliente.

### Cancelamento, disputa e avaliação (`OrderResolutionPanel`, reescrito)

- **Cancelamento direto** (`cancel`, só `DRAFT`/`OPEN`, antes de qualquer aceite) — novo, não existia na versão mock.
- **Cancelamento mútuo** (`cancel-request`/`cancel-response`) — só válido em `ACCEPTED`/`CLIENT_TRANSFERRED` na máquina real (a partir de `RECEIPT_CONFIRMED` o cashier já admitiu ter recebido, então virar mediação em vez de cancelamento de comum acordo — confirmado direto no `order-state-machine.ts` real, que bateu exatamente com o que o mock já simulava).
- **Abrir disputa** (`POST /orders/:id/dispute`, motivo obrigatório 10–2000 caracteres) — a resposta é a disputa criada, não a ordem; por isso usa o `refetchToken` em vez de atualizar a ordem com dados parciais.
- **Avaliação** (`POST /orders/:id/rating`) — achado real: o backend permite avaliar tanto `COMPLETED` quanto `CANCELLED` (`AVALIAVEIS` em `ratings.service.ts`), diferente do mock (só `COMPLETED`). Corrigido pra bater com a regra real. Sem campo "já avaliei" na ordem — o componente trata isso via estado local (`rated`) e o 409 do backend ("esta ordem já foi avaliada por você") quando aplicável. Ganhou campo de comentário opcional (o mock só tinha estrelas).

### `src/lib/mock/orders.tsx` — ações do ciclo principal removidas

Seguindo a regra já registrada (cada write que migra pra API real leva o mock equivalente junto, no mesmo commit): `CLIENT_TRANSFER`, `CASHIER_CONFIRM_RECEIPT`, `CASHIER_TRANSFER`, `CLIENT_CONFIRM`, `REQUEST_CANCEL`, `RESPOND_CANCEL`, `OPEN_DISPUTE`, `RATE` e `EXPIRE` saíram do reducer e do Context — sem chamador real restante (conferido por grep antes de remover). `REVIEW_DISPUTE`/`RESOLVE_DISPUTE`/`FREEZE_ORDER`/`UNFREEZE_ORDER` continuam (bucket "Administração & Mediação", ainda não migrado), assim como a lista `orders` em si (ainda alimenta `/admin/orders`, `/admin/reports`, `/reports`, `/profile`, `/admin/disputes` — nenhum desses foi tocado nesta rodada).

### Achado, não corrigido nesta rodada: identidade mock vs. identidade real numa mesma ordem

Toda checagem de "sou eu o cliente/caixeiro desta ordem" no app (`OrderActions`, `OrderResolutionPanel`, `OfferList`, etc.) compara `order.clientId`/`cashierId` contra `useMockSession().user.id` — a identidade "vista como" do `AccountSwitcher`, não a identidade real do JWT (que só o backend conhece, via Bearer token). Isso já era assim antes desta rodada (`orders/new/page.tsx` também usa `useMockSession()` pra tudo que não seja a chamada HTTP em si) — mas com leitura agora real, o efeito ficou visível pela primeira vez: uma conta real (`cliente@teste.local`) cria uma ordem de verdade, mas a tela mostra os controles de CAIXEIRO (porque o `clientId` real nunca bate com nenhum id de conta mock). Testado contra a API real: o botão "Aceitar ordem" aparece pro próprio autor, e o backend corretamente recusa (`"o autor da ordem não pode aceitá-la"`) — a UI mostra a mensagem de erro certa, mas o controle nem deveria aparecer. Não corrigido aqui porque a correção de verdade é aposentar `MockSessionProvider` inteiro (perfil, chaves PIX, limite de caixeiro — tudo ainda mock) em favor da identidade do JWT, o que é um card à parte, maior que este bucket. Registrado no [[Kanban]] como novo item de bloqueio.

Testado manualmente contra o ambiente de teste (`cliente@teste.local`): criação de ordem de venda real → detalhe carregando via `GET /orders/:id` (publicId `#13F2EE64`, timeline OK) → tentativa de autoaceite corretamente recusada pelo backend (422/403 conforme a regra) com toast específico. `npx tsc --noEmit` (projeto inteiro) e `npx eslint src` sem erros.

## Identidade real (JWT), reputação real e painel de mediação de disputas

Rodada seguinte: fecha os 3 itens que sobraram do bucket "Ordens & Ofertas" no [[Kanban]].

### Identidade real (`useAuth`) em vez da identidade mock nas checagens de papel

Achado da rodada anterior, corrigido aqui: `OrderDetail`, `/orders` (`orders/page.tsx`) e `OfferList` trocaram `useMockSession().user.id` pelo `user.id` real de `useAuth()` (JWT) na comparação `order.clientId`/`cashierId`. `OrderActions`/`OrderResolutionPanel` não precisaram mudar — já recebiam `viewerId` como prop, só a fonte no componente pai mudou. `cashierAvailableLimit` (limite pra aceitar) continua vindo de `useMockSession` — ainda não existe `GET /cashier/limit` real ligado no front, e isso é só um aviso client-side não-bloqueante (o backend já valida de verdade via 422).

### Reputação real (`GET /users/:id/ratings`) em `OfferList` e `OrderDetail`

Um fetch por cliente distinto na lista (não por linha renderizada), com cache em `Map` por sessão de componente. `counterparty-reputation-panel.tsx` — que dependia inteiramente do mock de ordens pra tempo de resposta/status online sintéticos (hash determinístico do id, sem equivalente real) — ficou órfão depois que `OrderDetail` passou a ler dados reais na rodada anterior (nenhum importador restante); deletado, junto com `counterpartyStats`/`riskAssessment` em `src/lib/mock/reputation.ts`. `getUserReputation` continua ali, ainda usado por `/profile` (bucket "Perfil & Configurações", não migrado).

### Painel de mediação de disputas (`/disputes`, `/disputes/[id]`)

Substitui `/admin/disputes` — mediação é papel `MEDIATOR`, não `ADMIN`, então saiu de dentro do admin pra uma rota própria (adicionada à navegação principal, `nav-items.ts`). Sem "assumir revisão" explícito como no protótipo mock: o backend real designa o mediador com menos fila já na abertura da disputa (`designarMediador`, `disputes.service.ts`), e a primeira recomendação assume o caso se ainda não tinha dono — não existe endpoint separado pra isso.

- **Fila** (`GET /disputes`) — o backend já filtra por conta: mediador vê as próprias + as sem dono, parte vê as das próprias ordens.
- **Detalhe** (`GET /disputes/:id`) — evidências, mensagens (já filtradas pelo público de quem pede — canal reservado mediador↔cliente não aparece pro caixeiro, e vice-versa) e decisões.
- **Evidência** (`POST /disputes/:id/evidence`, multipart) — suporte a `FormData` novo no cliente HTTP (`src/lib/api/client.ts`): detecta `body instanceof FormData` e pula `JSON.stringify`/`Content-Type` manual (o browser define o boundary do multipart sozinho). Primeira rota do app a precisar disso — upload de comprovante no chat (bucket "Chat & Comprovantes") vai reaproveitar o mesmo suporte quando for a vez dele.
- **Mensagens** (`POST /disputes/:id/messages`) — só o mediador escolhe o público (`ALL`/`MEDIATOR_CLIENT`/`MEDIATOR_CASHIER`); partes sempre mandam `ALL` (campo escondido da UI pra quem não é mediador).
- **Decisão** (`POST /disputes/:id/decision`) — dois mediadores: `kind: "RECOMMENDATION"` primeiro, depois `kind: "APPROVAL"` de **outra** conta confirmando o mesmo `resolution`. A UI calcula `pendingRecommendation` (última recomendação sem aprovação correspondente em `decisions`) pra decidir se mostra o formulário de recomendar ou o de aprovar, e esconde o de aprovar pra quem recomendou (o backend também recusa com 403, isso é só UX — não deixar clicar num botão que vai falhar por regra conhecida).

`src/lib/mock/disputes.tsx` deletado (sem importador restante); `MockDisputesProvider` removido de `(dashboard)/layout.tsx`; `REVIEW_DISPUTE`/`RESOLVE_DISPUTE` removidos do reducer de `src/lib/mock/orders.tsx` (mesma regra de sempre — mock sai no mesmo commit da migração). `FREEZE_ORDER`/`UNFREEZE_ORDER` continuam (bucket "Administração & Mediação", ainda não migrado, usa a mesma lista `orders` mock pra outros fins).

### Achado crítico: o ambiente de teste está desatualizado em relação ao código-fonte

Testado contra `https://api.163-176-220-125.sslip.io`: `GET /disputes` devolve **404 "Cannot GET /disputes"**, não 401 — ou seja, a rota não existe nesse servidor, não é uma questão de autenticação. Conferido contra o Swagger ao vivo (`/docs-json`) da própria VM: o servidor expõe só `auth` (login/logout/mfa verify/refresh/register — **sem** `/auth/mfa` de enrollment), `cashier/collateral`, `cashier/limit`, `health` e o módulo `orders` completo (incluindo `/orders/:id/pix`, todo o ciclo de transferência, cancelamento e `/orders/:id/messages`). **Não existem no servidor ao vivo**: `ratings` (nem `/orders/:id/rating` nem `/users/:id/ratings` — a rodada anterior, que implementou avaliação, também está OK no código mas não testável ao vivo por este mesmo motivo), `disputes` (nenhuma rota), `admin/*`, `kyc/*`, `uploads`.

Ou seja: o repositório `marinsprosper-api` no GitHub (branch `main`, de onde vieram os DTOs/controllers usados nesta implementação) está bem à frente do que está de fato rodando na VM de teste. Todo o código deste bucket foi escrito e conferido contra o código-fonte real do repositório — não contra suposição — mas **avaliação, disputas, admin, KYC e uploads não são testáveis nem utilizáveis contra o ambiente de teste atual até a VM ser redeployada** com a versão atual do backend. Isso não é um bug do frontend; é uma pendência de infraestrutura/deploy do backend, registrada no [[Kanban]].

Testado manualmente contra a API real (`mediador@teste.local`): login OK, `/disputes` carrega e mostra a mensagem de erro específica (`Backend recusou a listagem: Cannot GET /disputes`) em vez de travar a tela — confirma que o tratamento de erro da listagem funciona mesmo quando o endpoint não existe ainda. Build de produção (`npx next build`) e `npx eslint src` sem erros; verificação funcional completa (fila carregando disputas de verdade, anexar evidência, decidir) fica pendente até o redeploy do backend.
