...
- **Criação de ordem** (`src/lib/orders/api.ts`, `types.ts`, `orders/new/page.tsx`) — `POST /orders` numa chamada só, `publish: true` sempre (não existe endpoint `/publish` separado). Corpo: `side` (`CLIENT_BUYS_ASSET`/`CLIENT_SELLS_ASSET`, derivado de `Order.type`), `asset: "USDT"`, `assetAmount`/`rate` como string (do quote local), `clientTronAddress` obrigatório em compra (formato validado na UI via `TRON_ADDRESS_PATTERN`, checksum de verdade só o backend confere). **201 confirmado contra a API real** — `fiatAmount` da resposta bate com `assetAmount * rate`, validando o mapeamento inferido (`assetAmount` = quantidade de USDT, não o valor em BRL). `BackendOrder` em `types.ts` documenta o formato de resposta real capturado. Chamada real roda em paralelo à criação no mock (dual-write transicional): `GET /orders`/`/orders/:id` ainda não existem no front, então nenhuma tela lê a ordem real criada — lista e detalhe continuam 100% do "backend fake" até essas leituras também ficarem reais.
- **Aceite de ordem** (`offer-list.tsx`) — `POST /orders/:id/accept`, sem corpo, só `Idempotency-Key`. **422 real confirmado** ("saldo de colateral desatualizado; sincronize com o contrato antes de aceitar") tratado como erro específico via `error.message`, não genérico. Mesmo dual-write transicional do card de criação — os ids de `orders` no mock não existem no backend real, batem 404 lá até a listagem também ficar real.
