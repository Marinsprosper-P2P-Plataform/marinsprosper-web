...
Todos os itens de código do bucket "Fundação" do Kanban já existem, e não há mais nenhuma pendência de coordenação com o backend — ver correção logo abaixo sobre o falso alarme de CORS.

- **Cliente HTTP** (`src/lib/api/client.ts`, `config.ts`, `errors.ts`, `types.ts`) — `apiFetch`/`api.{get,post,patch,delete}`, base URL sempre via `NEXT_PUBLIC_API_URL` (nunca `localhost`), `Decimal = string` como lembrete de tipo pra nunca converter dinheiro, `ApiError` (`isNotFound`/`isConflict`/`isUnauthorized`) separado de `ApiNetworkError` (falha de rede/config antes de qualquer resposta chegar — inclui `NEXT_PUBLIC_API_URL` ausente, não só problema de rede de verdade).
...
- **Autenticação real** (`src/lib/auth/`) — login/register/refresh/logout/MFA (verify + recovery) contra o backend de teste, sessão em `localStorage` (`storage.ts`), claims do `accessToken` lidas só pra UI via `jwt.ts` (nunca decide autorização sozinho), interceptor de 401 (`client.ts` → `auth-token.ts` → `AuthProvider`) que tenta um refresh e só chama `notifySessionExpired()` se ele também falhar. `AuthProvider` entra no layout raiz (`src/app/layout.tsx`), ao lado do `ThemeProvider` — **não mexe** no `MockSessionProvider`/`AccountSwitcher` do bucket `(dashboard)`, que continuam sendo a identidade "vista como" pro resto do app ainda mockado (ordens, carteira etc.).
  ...
  - Testado manualmente contra `https://api.163-176-220-125.sslip.io` em 2026-08-14: login funciona de ponta a ponta com `.env.local` configurado (ver correção abaixo) — token real emitido, claims (`sub`/`email`/`roles`) exatamente no formato que `src/lib/auth/types.ts` já assumia (`roles: string[]`, ex. `["CLIENT"]`, contas com mais de um papel vêm com array de mais de um item, ex. `["CASHIER","CLIENT"]`).
- **Remapeamento da máquina de estados** (`src/lib/order-status-map.ts`, `src/types/order.ts`) — `mapBackendOrderStatus(status, viewerRole)` traduz os 11 estados reais (tabela desta seção, §2) pro `OrderStatus` do front dependendo de quem olha: os 4 pares `AWAITING_*` não são estado próprio no backend, são a mesma etapa vista por um papel que ainda precisa agir (ou só está esperando) — a função decide qual rótulo mostrar por papel, ver comentário no arquivo pro raciocínio de cada par. `ORDER_HAPPY_PATH_STEPS` agrupa os pares num só step visual e tira `RESERVED` do stepper (nunca existiu no backend, nem no reducer mock chega a atribuí-lo). `OrderTimeline` foi reescrito pra iterar os grupos — confirmado visualmente contra os pedidos seed: 6 steps em vez de 11, ativo/concluído corretos. `DISPUTED` do backend mapeia pra `DISPUTE_OPEN` por padrão (as 3 fases do front não existem no schema real ainda, ver nota da tabela). Ainda sem chamador de verdade — usada só nesta doc pra planejar; o bucket "Ordens & Ofertas" é quem vai plugar quando `GET /orders/:id` substituir o mock.
- **Criação de ordem** (`src/lib/orders/api.ts`, `types.ts`, `orders/new/page.tsx`) — `POST /orders` numa chamada só, `publish: true` sempre (não existe endpoint `/publish` separado). Corpo: `side` (`CLIENT_BUYS_ASSET`/`CLIENT_SELLS_ASSET`, derivado de `Order.type`), `asset: "USDT"`, `assetAmount`/`rate` como string (do quote local), `clientTronAddress` obrigatório em compra (formato validado na UI via `TRON_ADDRESS_PATTERN`, checksum de verdade só o backend confere). **201 confirmado contra a API real** — `fiatAmount` da resposta bate com `assetAmount * rate`, validando o mapeamento inferido (`assetAmount` = quantidade de USDT, não o valor em BRL). `BackendOrder` em `types.ts` documenta o formato de resposta real capturado. **Sem fallback em mock**: `createOrder` foi removido de `src/lib/mock/orders.tsx` — se o backend recusar, a ordem não é criada, ponto (ver princípio "backend nunca localhost, tudo via API real"). `GET /orders`/`/orders/:id` ainda não existem no front, então o redirect pós-criação leva a uma tela que mostra "Ordem não encontrada" — a ordem existe de verdade no backend, só falta a leitura.
- **Aceite de ordem** — `POST /orders/:id/accept`, sem corpo, só `Idempotency-Key`. **422 real confirmado** ("saldo de colateral desatualizado; sincronize com o contrato antes de aceitar") tratado como erro específico via `error.message`, não genérico. Ligado nos dois pontos que aceitam ordem: `offer-list.tsx` (`/offers` e aba "Disponível" de `/orders`) e `AcceptControl` em `order-actions.tsx` (botão "Aceitar ordem" do detalhe da ordem) — os dois sem fallback em mock (`acceptOrder` também removido de `src/lib/mock/orders.tsx`). Ids de `orders` no mock não existem no backend real e batem 404 lá até `GET /orders` também ficar real; tratado como erro específico, não genérico, igual ao 422.

### Correção (2026-08-14): não existe pendência de CORS

O item "pedir pro time de backend liberar `CORS_ORIGINS`" que esteve nesta doc e no Kanban (bucket Fundação) era **falso alarme**. A causa real do "não foi possível contatar o servidor" no teste de `/login` daquele dia era `.env.local` ausente na sessão de teste — arquivo gitignored (`.gitignore` já cobre `.env*`), cada ambiente de desenvolvimento precisa copiar `.env.example` pra `.env.local` antes de rodar `npm run dev`; sem isso `NEXT_PUBLIC_API_URL` fica vazia, `getApiBaseUrl()` lança, e o catch do cliente HTTP embrulha isso como `ApiNetworkError` com a mesma mensagem genérica que uma falha de CORS de verdade daria — os dois casos eram indistinguíveis pelo toast sozinho.

Com `.env.local` configurado, testado e confirmado nesta mesma sessão, sem nenhuma mudança em `CORS_ORIGINS`:
- `POST /auth/login` — 200, tokens reais emitidos, papéis batendo com o esperado.
- `POST /orders` — 201, `fiatAmount` retornado bate com `assetAmount * rate` (confirma o mapeamento inferido em §5/`src/lib/orders/types.ts`).
- `POST /orders/:id/accept` — 422 real ("saldo de colateral desatualizado; sincronize com o contrato antes de aceitar"), exatamente o tipo de erro específico que o card de aceite pedia pra tratar.

Não há mais nenhum bloqueio de coordenação com o time de backend registrado neste vault.
...
- **`CORS_ORIGINS` já cobre o `localhost` de dev** — confirmado contra chamadas reais em 2026-08-14 (login, criação e aceite de ordem), sem precisar de nenhuma mudança do time de backend. Vai precisar de ajuste quando o domínio do Vercel entrar em cena (deploy), mas o `localhost` do dia a dia já funciona.
- **`NEXT_PUBLIC_API_URL`/`NEXT_PUBLIC_WS_URL`** (`.env.example`, na raiz deste repo) já apontam pro ambiente de teste acima — não pra `localhost`. **Cada ambiente de desenvolvimento precisa copiar `.env.example` pra `.env.local`** (gitignored, não vem pronto) antes de rodar `npm run dev` — sem isso `NEXT_PUBLIC_API_URL` fica vazia e toda chamada real falha com uma mensagem genérica de erro de rede, fácil de confundir com CORS (foi exatamente essa confusão que gerou o falso alarme registrado nesta doc até 2026-08-14). Vars precisam ser trocadas de novo quando o backend for pra produção de verdade (Cloud Run, ver README do backend).
...
- **CORS com allowlist obrigatória em produção** — `NEXT_PUBLIC_API_URL` do front precisa estar cadastrado em `CORS_ORIGINS` do backend; wildcard é rejeitado quando há credenciais. `localhost` de dev já está coberto (ver §0); vai precisar de ajuste só quando o domínio do Vercel entrar em cena.
